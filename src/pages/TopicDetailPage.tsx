...
export default function TopicDetailPage() {
  ...
  const progress = useMemo(() => {
    ...
    const solvedQuestions = Array.isArray(userProgress.solvedQuestions)
      ...
      : Object.keys(userProgress.solvedQuestions || {}).filter(
          (id) => userProgress.solvedQuestions[id]
        )
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    <p className="font-semibold text-gray-900 dark:text-white">
                      {progress === null
                        ...
                        : `${progress?.badgeLevel
                            .charAt(0)
                            .toUpperCase()}${progress?.badgeLevel.slice(
                            1
                          )} Badge`}
                    </p>
                    {nextBadge && (
                      <p className="text-sm text-gray-600 dark:text-gray-300">
                        {nextBadge.remaining} more points for {nextBadge.level}{' '}
                        badge
                      </p>
                    )}
                  </div>
                    ...
                    <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-2 mt-2">
                      <div
                        ...
                        style={{
                          width: `${Math.min(
                            100,
                            ((progress?.points || 0) / topic.totalPoints) * 100
                          )}%`,
              ...
              <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="font-semibold text-gray-900 dark:text-white">
                      {topicProgress?.solvedQuestions?.length || 0} /{' '}
                      {topic.questions.length} completed
                    </p>
                    <p className="text-sm text-gray-600 dark:text-gray-300">
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredQuestions.map((question) => {
                  ...
                  const isSolved =
                    topicProgress?.solvedQuestions?.includes(question.id) ||
                    false
